...
impl SynthUpdater for PulseUpdater {
	fn update<'a>(
		...
		for sound in sounds {
			...
			let sound_until = Duration::from_ticks(time_range.0.as_ticks() % sound.period.as_ticks());
			...
			until_next = min(time_range.1.checked_duration_since(sound_next_time).unwrap_or(Duration::from_ticks(0)), until_next);
		}
		...
		let off_period = max(
			Duration::from_micros(synth_config.max_note_half_delay_us),
			until_next.checked_sub(on_period).unwrap_or(Duration::from_ticks(0)),
		);
